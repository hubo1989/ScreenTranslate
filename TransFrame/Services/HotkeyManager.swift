...
actor HotkeyManager {
    ...
    /// - Throws: TransFrameError.hotkeyRegistrationFailed if registration fails
    func register(
        keyCode: UInt32,
        modifiers: UInt32,
        ...
        guard status == noErr, let ref = hotKeyRef else {
            throw TransFrameError.hotkeyRegistrationFailed(keyCode: keyCode)
        }
    ...
    private func installEventHandler() throws {
        ...
        guard status == noErr else {
            throw TransFrameError.hotkeyRegistrationFailed(keyCode: 0)
        }
