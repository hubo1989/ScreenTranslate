...
actor ScreenDetector {
    ...
    /// - Throws: TransFrameError if enumeration fails or no displays found
    func availableDisplays() async throws -> [DisplayInfo] {
        ...
        guard !scDisplays.isEmpty else {
            throw TransFrameError.captureError(message: "No displays available")
        }
    ...
    /// - Throws: TransFrameError if no primary display found
    func primaryDisplay() async throws -> DisplayInfo {
        ...
        guard let primary = displays.first(where: { $0.isPrimary }) ?? displays.first else {
            throw TransFrameError.captureError(message: "No primary display available")
        }
    ...
    /// - Throws: TransFrameError.displayNotFound if not found
    func display(withID displayID: CGDirectDisplayID) async throws -> DisplayInfo {
        ...
        guard let display = displays.first(where: { $0.id == displayID }) else {
            throw TransFrameError.displayNotFound(displayID)
        }
