...
actor CaptureManager {
    ...
    /// - Throws: TransFrameError if capture fails
    func captureFullScreen(display: DisplayInfo) async throws -> Screenshot {
        // Prevent concurrent captures
        guard !isCapturing else {
            throw TransFrameError.captureError(message: "Capture already in progress")
        }
        isCapturing = true
    ...
    /// - Throws: TransFrameError if enumeration fails
    ...
    /// - Throws: TransFrameError if display not found or content retrieval fails
    private func getSCDisplay(for display: DisplayInfo) async throws -> SCDisplay {
        ...
        guard let scDisplay = scContent.displays.first(where: { $0.displayID == display.id }) else {
            throw TransFrameError.displayDisconnected(displayName: display.name)
        }
        return scDisplay
    }
