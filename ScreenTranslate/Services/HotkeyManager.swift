...
actor HotkeyManager {
    ...
    /// - Throws: ScreenTranslateError.hotkeyRegistrationFailed if registration fails
    func register(
        keyCode: UInt32,
        modifiers: UInt32,
        ...
        guard status == noErr, let ref = hotKeyRef else {
            throw ScreenTranslateError.hotkeyRegistrationFailed(keyCode: keyCode)
        }
    ...
    private func installEventHandler() throws {
        ...
        guard status == noErr else {
            throw ScreenTranslateError.hotkeyRegistrationFailed(keyCode: 0)
        }
